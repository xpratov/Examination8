...
const SingleInvoices = ({target,id,author,paymentTerms,price,date,type,}: {target: string;id: string;author: string;date: string;paymentTerms: string;price: string;type: "paid" | "draft" | "pending" | string;}) => {
  ...
  const getDueDate = (date: string, dueDate: string) => {
    ...
    const dueDay = isNaN(+dueDate.slice(0, 2))? +dueDate.slice(0, 1): +dueDate.slice(0, 2);
    if (month.at(0) === "0") {
      month = month.at(-1)!;
    }
  ...
  return (
    <div onClick={handleClick} className="w-full border active:border-gray max-w-lg hover:opacity-80 lg:cursor-pointer md:gap-0 md:flex md:justify-between md:items-center grid grid-cols-[auto_auto] dark:bg-darkBlue items-center md:grid-rows-1 rounded-xl gap-6 bg-white p-6 shadow-[rgba(72,84,159,0.1)] md:max-w-[750px] grid-rows-2  justify-between  ">
      ...
      <article className="grid  md:flex md:items-center md:gap-16 md:justify-between gap-2  dark:text-white">
      ...
      <InVoiceTypes type={type} />
        <img alt="back" src="/arrow-right-icon.svg" className="hidden md:block md:w-5 md:h-5  lg:cursor-pointer"/>
    </div>
  );
};
