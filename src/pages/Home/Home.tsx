...
import InVoicesController from "./components/ControlInvoices";
import { createPortal } from "react-dom";
import NewInVoice from "../../components/CreateInvoice";
...
const Home = () => {
  ...
  return (
    ...
    >
      ...
      >
        {!loading ? (
          <main className="w-ful pt-8 lg:px-0 px-6 overflow-x-hidden lg:max-w-[730px] lg:mx-auto  flex lg:py-16 flex-col items-center lg:max-h-dvh md:pt-14">
            <InVoicesController filter={filter} setFilter={setFilter}inVoices={trueData?.length ? trueData.length : 0}/>
            <InVoices ids={ids}datas={
                filter === "all"
                  ? trueData
                  : trueData?.filter(
      ...
      </div>                                                                                                                                                                                                                    {createPortal(<NewInVoice />, document.body)}
    </ShowNewInVoice.Provider>
  );
};
