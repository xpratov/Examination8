...
const ToSection = ({form,date,}: {form: (a: string, b: object) => any;date?: string;}) => {
  useEffect(() => {}, []);
  const getDate = () => {
    const date = new Date();
    ...
    <div className="grid gap-6 max-w-full mb-10">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="grid opacity-70">
          ...
          <input type="date" value={date ? date : getDate()}
            disabled className="border bg-transparent border-lightGray dark:border-lightBlue rounded-lg py-[18px] px-5 block focus:border-purpleLight outline-none dark:bg-darkBlue dark:text-white  w-full"
          />
        </div>
        <div className="grid"> 
          ...
          id="select"
            className="border bg-transparent border-lightGray rounded-lg py-[18px] px-5 block focus:border-purpleLight outline-none dark:border-lightBlue dark:bg-darkBlue dark:text-white  w-full"
          >
