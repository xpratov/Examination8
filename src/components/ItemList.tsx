...
import SpecialInput from "./Input2";

const ItemList = ({items,setItems,}: {items: any[];setItems: (a: any) => void;}) => {
  const handleClick = () => {
    setItems([
      ...items,
      {
  ...
  return (
    <div className="mt-[69px] w-full">
            ...
            (
              ...
              <div key={i} className="grid gap-6 md:grid-cols-[2fr_3fr]">
                <div className="grid gap-2">
                  ...
                  <SpecialInput setData={setItems}id={i}name="name"array={items}type="name"/>
                </div>
                <div className="grid grid-cols-[3fr_4fr_6fr] gap-4">
                  <div className="grid gap-2">
                    <Label hidden>Qty.</Label>
                    <SpecialInput id={i} setData={setItems}type="number"array={items}name="count"/>
                  </div>
                  <div className="grid gap-2">
                    <Label hidden>Price</Label>
                    <SpecialInput setData={setItems} id={i} type="number" array={items} name="price" />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <div className="py-4 flex justify-between">
                      ...
                      <button type="button"
                        ...
                        className="cursor-pointer lg:hover:text-orange text-gray "
                      >
                        <svg
                          width="20"
                          viewBox="0 0 13 16"
      ...
      <button onClick={handleClick} type="button" className="w-full py-[18px] bg-calc dark:bg-lightBlue rounded-3xl lg:hover:opacity-75  font-bold text-blue dark:text-lightGray">
        + Add Item
      </button>
    </div>
  );
